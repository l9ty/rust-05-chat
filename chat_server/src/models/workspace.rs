use chat_core::{RowID, User, Workspace};
use sqlx::PgPool;
...
mod tests {
    ...
    async fn t_create(pool: PgPool) {
        ...
        let ret = find_by_id(&pool, 1).await.unwrap();
        assert!(ret.is_none());

        // find by name should return none
        let ret = find_by_name(&pool, "test").await.unwrap();
        assert!(ret.is_none());

        // create should work
        let ws = create(&pool, "test", 1).await.unwrap();
        assert!(ws.is_some());

        // create again should return none
        let ret = create(&pool, "test", 1).await.unwrap();
        assert!(ret.is_none());
        ...
        let ret = update_owner(&pool, 1, 2).await;
        assert!(ret.is_ok());

        // list all users should work
        let ret = list_all_users(&pool, 1).await;
        assert!(ret.is_ok());
    }
}
